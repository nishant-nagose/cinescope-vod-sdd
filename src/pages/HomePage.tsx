...
import { CAROUSEL_POOL } from '../config/carouselPool'
...
export const HomePage = () => {
  ...
  // Filter pool by content-type toggle and selected category
  const filteredPool = CAROUSEL_POOL.filter(config => {
    const typeOk =
      (config.type === 'movies' && (contentType === 'movies' || contentType === 'all')) ||
      (config.type === 'shows'  && (contentType === 'shows'  || contentType === 'all'))
    if (!typeOk) return false
    // When category is active, only show carousels whose genreKey matches exactly
    if (activeGenreKey !== null && config.genreKey !== activeGenreKey) return false
    return true
  })

  const visiblePool = filteredPool.slice(0, visibleCount)
  const hasMoreCarousels = visibleCount < filteredPool.length
    ...
    const observer = new IntersectionObserver(
      entries => {
        if (entries[0].isIntersecting) {
          setVisibleCount(n => Math.min(n + LOAD_BATCH, filteredPool.length))
        }
      },
      { rootMargin: '600px' }
    )
  ...
  }, [hasMoreCarousels, filteredPool.length])
